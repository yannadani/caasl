...
def log_performance(
    itr, batch, discount, prefix="Evaluation", return_cumulative=False, save_path=None
):
    ...
    for trajectory in batch.split():
        ...
        observations.append(trajectory.last_observations)
        masks.append(trajectory.masks)
        if return_cumulative:
            cumulative_rewards.append(torch.cumsum(trajectory.rewards, 0))
    ...
    cumulative_rewards = torch.stack(cumulative_rewards).cpu().mean(0)
    observations = torch.stack(observations).cpu().numpy()
    masks = torch.stack(masks).cpu().numpy()
    if save_path is not None and itr is not None:
        if itr % 2500 == 0:
            os.makedirs(f"{save_path}/{prefix}", exist_ok=True)
            pkl.dump(
                {"obs": observations, "masks": masks},
                open(f"{save_path}/{prefix}/{itr}.pkl", "wb"),
            )
    with tabular.prefix(prefix + "/"):
        ...
        tabular.record("UpperQuartileReturn", np.percentile(undiscounted_returns, 75))
        tabular.record("LowerQuartileReturn", np.percentile(undiscounted_returns, 25))
    ...
    if return_cumulative:
        return undiscounted_returns, cumulative_rewards
