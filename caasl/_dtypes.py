...
class TimeStepBatch:
    ...
    def concatenate(cls, *batches):
        ...
        return cls(
            ...
            next_masks=torch.cat([batch.next_masks for batch in batches]),
            env_infos=env_infos,
    ...
    def split(self) -> List["TimeStepBatch"]:
        ...
        for i in range(len(self.rewards)):
            time_step = TimeStepBatch(
                episode_infos={
                    k: torch.as_tensor([v[i]]) for (k, v) in self.episode_infos.items()
                },
                ...
                env_infos={
                    k: torch.as_tensor([v[i]]) for (k, v) in self.env_infos.items()
                },
                agent_infos={
                    k: torch.as_tensor([v[i]]) for (k, v) in self.agent_infos.items()
                },
...
class EpisodeBatch(TimeStepBatch):
    ...
    def __init__(
        ...
        if not torch.is_tensor(last_observations):
            raise ValueError(f"last_observations is not a pytorch tensor")
        ...
        check_timestep_batch(
            self, torch.Tensor, ignored_fields={"next_observations", "episode_infos"}
        )
    ...
    def concatenate(cls, *batches):
        ...
        return cls(
            ...
            env_spec=batches[0].env_spec,
            observations=torch.cat([batch.observations for batch in batches]),
            last_observations=torch.cat([batch.last_observations for batch in batches]),
            masks=torch.cat([batch.masks for batch in batches]),
            ...
            episodes.append(
                {
                    ...
                    "next_masks": torch.cat(
                        [self.masks[1 + start : stop], self.last_masks[i].unsqueeze(0)]
                    ),
                }
            )
        return episodes
    ...
    def from_list(cls, env_spec, paths):
        ...
        if all(
            ...
            last_observations = torch.as_tensor([p["observations"][-1] for p in paths])
            observations = torch.cat([p["observations"][:-1] for p in paths])
        else:
