...
class Trainer:
    ...
    def __init__(self, snapshot_config, wandb=False):
    ...
    def save(self, epoch, best=False, best_return=None):
        ...
        self._snapshotter.save_itr_params(epoch, params, best=best)

        logger.log("Saved")
        logger.log("best_return: %s" % best_return)

    def restore(
        self, from_dir, replay_buffer=None, sampler=None, from_epoch="last", alpha=None
    ):
        ...
        set_seed(self._seed)
        if replay_buffer is not None and not hasattr(saved["algo"], "replay_buffer"):
            saved["algo"].replay_buffer = replay_buffer
        if sampler is not None and not hasattr(saved["algo"], "_sampler"):
            saved["algo"]._sampler = sampler(
                agents=saved["algo"].policy, envs=saved["env"]
            )
        if alpha is not None:
            saved["env"]._use_automatic_entropy_tuning = False
            saved["algo"]._fixed_alpha = alpha
    ...
    def log_diagnostics(self, pause_for_plot=False):
        ...
        # logger.log(tabular)
        if self.wandb:
            log_dict = tabular.as_dict
            log_dict["TotalEnvSteps"] = self._stats.total_env_steps.item()
            wandb.log(log_dict)
        if self._plot:
    ...
    def step_epochs(self):
        ...
        for epoch in range(self._train_args.start_epoch, n_epochs):
            ...
            with logger.prefix("epoch #%d | " % epoch):
                ...
                if epoch % self._snapshotter.snapshot_gap == 0:
                    self.save(epoch)
