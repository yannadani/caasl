...
def set_rng_seed(rng_seed, global_=True):
    """
...
def torch_nanmin(tensor, dim=None, keepdim=False):
    ...
    for d in dim or []:
        output = output.min(dim=d, keepdim=True)[0]
    output[output == max_value] = float("nan")
    return output
...
def warn_if_nan(value, msg=""):
    ...
    if torch_isnan(value):
        warnings.warn(
            "Encountered NaN{}".format((": " if msg else ".") + msg), stacklevel=2
        )
...
def warn_if_inf(value, msg="", allow_posinf=False, allow_neginf=False):
    ...
    if (not allow_posinf) and (
        ...
        warnings.warn(
            "Encountered +inf{}".format((": " if msg else ".") + msg), stacklevel=2
        )
