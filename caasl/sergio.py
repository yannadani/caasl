...
from . import set_rng_seed, wrap_experiment
...
from .models.causal_experiment_model import GRNSergioModel
from .policies import AdaptiveTransformerTanhGaussianPolicy
from .q_functions.adaptive_mlp_q_function import AdaptiveMLPQFunctionDoCausal
from .replay_buffer import PathBuffer
from .sampler.local_sampler import LocalSampler
from .sampler.vector_worker import VectorWorker
from .utils.ood_evaluation_loader import OODEvaluationLoader
...
def main(
    ...
    config_path="caasl/configs/sergio_train.yaml",
):
    ...
    wandb = wandb if wandb is not None else defaults.get("wandb", False)
    norm_rewards = (
        norm_rewards
    ...
    intervention_type = (
        ...
        else defaults.get("intervention_type", "kout")
    )
    shared_encoder = (
        shared_encoder
    ...
    # Handle wandb initialization if wandb is True
    if wandb:
        import wandb
    ...
    def sac_source(
        ...
        trainer = Trainer(snapshot_config=ctxt, wandb=wandb)
        ...
        if os.path.exists(os.path.join(ctxt.snapshot_dir, "params.pkl")):
            ...
            ood_loader = OODEvaluationLoader(config_path)
            eval_env = ood_loader.create_main_eval_environment(
                budget=budget, data_seed=data_seed, **kwargs
            )
            eval_ood_envs = ood_loader.create_ood_environments(
                budget=budget, data_seed=data_seed, **kwargs
            )
