...
from .._dtypes import TimeStepBatch
...
class NMCBuffer:
    ...
    def add_episode_batch(self, episodes):
        ...
        for eps in episodes.split():
            path = dict(
                ...
                next_mask=torch.cat([eps.masks[1:], eps.last_masks.unsqueeze(dim=0)]),
    ...
    def add_path(self, path):
        ...
        self._transitions_stored = min(
            self._capacity, self._transitions_stored + self.path_len
        )
    ...
    def _get_or_allocate_key(self, key, array):
        ...
        if buf_arr is None:
            buf_arr = torch.zeros(
                (self._capacity,) + array.shape[1:], dtype=array.dtype
            )
