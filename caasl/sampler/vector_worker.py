...
class VectorWorker(DefaultWorker):
    ...
    def pad_observation(self, obs):
        ...
        pad_shape[1] = (
            self.num_init_obs
            + self._max_episode_length * self.batch_size
            - pad_shape[1]
        )
    ...
    def start_rollout(self):
        """Begin a new rollout."""
        self._path_length = 0
        self._prev_obs, prev_ = self.env.reset(n_parallel=self._n_parallel)
        self.init_reward = prev_["reward"]
    ...
    def step_rollout(self, deterministic):
        ...
        if self._path_length < self._max_episode_length:
            a, agent_info = self.agent.get_actions(self._prev_obs, self._prev_mask)
            ...
            if not env_step.terminal:
                ...
                return False
        self._lengths = self._path_length * torch.ones(
            self._n_parallel, dtype=torch.int
        )
        self._last_observations.append(self._prev_obs)
        self._last_masks.append(self._prev_mask)
        print(f"Step rollout time: {time.time() - start_time}")
        return True
    ...
    def collect_rollout(self):
        ...
        observations = torch.cat(
            torch.split(torch.stack(self._observations, dim=1), 1), dim=1
        ).squeeze(0)
        self._observations = []
        ...
        masks = torch.cat(
            torch.split(torch.stack(self._masks, dim=1), 1), dim=1
        ).squeeze(0)
        # masks = self._masks
        self._masks = []
        ...
        self._last_masks = []
        actions = torch.cat(
            torch.split(torch.stack(self._actions, dim=1), 1), dim=1
        ).squeeze(0)
        self._actions = []
        self._rewards[0] = self._rewards[0] + self.init_reward
        rewards = torch.cat(
            torch.split(torch.stack(self._rewards, dim=1), 1), dim=1
        ).squeeze(0)
        self._rewards = []
        terminals = torch.cat(
            torch.split(torch.stack(self._terminals, dim=1), 1), dim=1
        ).squeeze(0)
        self._terminals = []
        ...
        for k, v in agent_infos.items():
            agent_infos[k] = torch.cat(
                torch.split(torch.stack(v, dim=1), 1), dim=1
            ).squeeze(0)
        ...
        for k, v in env_infos.items():
            if torch.is_tensor(v[0]):
                ...
                env_infos[k] = torch.cat(
                    torch.split(torch.as_tensor(v).float() + zs, 1), dim=1
                ).squeeze(0)
    ...
    def rollout(self, deterministic=False):
        ...
        start_time = time.time()
        self.start_rollout()
        print(f"Start rollout time: {time.time() - start_time}")
        time_step_start = time.time()
        while not self.step_rollout(deterministic):
            pass
        print(f"Time step time: {time.time() - time_step_start}")
        time.time()
